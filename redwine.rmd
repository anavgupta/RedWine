...
redwine$quality.group <- as.factor(cut(redwine$quality.num, c(2, 4, 6)))
```
...
```{r echo=FALSE}
qplot(data = redwine, x = fixed.acidity, fill = I('#f79420'), binwidth = .05) +
  xlim(4, 14)
```
...
qplot(data = subset(redwine, volatile.acidity <= quantile(redwine$volatile.acidity,
      .99)), x = volatile.acidity, fill = I('#f79420'), binwidth = .01)
```
...
```{r echo=FALSE}
qplot(data = redwine, x = citric.acid, fill = I('#f79420'), binwidth = .005) 
```
...
```{r echo=FALSE}
ggplot(data = subset(redwine, residual.sugar <= quantile(redwine$residual.sugar,
...
ggplot(data = redwine, aes(x = chlorides)) +
  geom_histogram(fill = I('#f79420'), binwidth = .005)
...
ggplot(data = subset(redwine, chlorides <= quantile(redwine$chlorides, .95)),
       aes(x = chlorides)) +
  geom_histogram(fill = I('#f79420'), binwidth = .0005) 
```
...
ggplot(aes(free.sulfur.dioxide), data = subset(redwine, free.sulfur.dioxide <=
  ...
  geom_histogram(fill = I('orange'), binwidth = .5)
```
...
ggplot(data = redwine, aes(x = density)) +
  geom_histogram(fill = I('Orange'), binwidth = .00005) +
...
ggplot(data = redwine, aes(x = pH)) +
  geom_histogram(fill = I('Orange'), binwidth = .005) +
...
```{r echo=FALSE}
qplot(data = redwine, x = sulphates, fill = I('orange'), binwidth = .005) +
...
ggplot(data = redwine, aes(x = alcohol)) +
  geom_histogram(fill = I('orange'), binwidth = .1) +
...
```{r echo=FALSE}
ggcorr(subset(redwine, select = c(-X, -quality, -quality.group)), label = TRUE)
...
ggpairs(subset(redwine, select = -X),
...
```{r echo=FALSE}
ggplot(data = redwine, aes(quality, volatile.acidity)) +
  geom_point(alpha = 1/5, color = I('orange'), position = 'jitter') +
  scale_y_continuous(breaks = seq(0.3, 1.6, .2))
```

We pretty much can't tell any thing from the above scatter plot. 
Lets create a box plot for them.
...
```{r echo=FALSE}
redwine %>%
  group_by(quality, gr = cut(redwine$residual.sugar, c(0, 1, 2, 3, 4, 8, 16))) %>%
  summarise(n = n())
```
...
ggplot(data = redwine, aes(x = chlorides)) +
  geom_histogram(aes(fill = quality.group), binwidth = .001) +
  scale_x_continuous(limits = c(0, quantile(redwine$chlorides, 0.95)))
```
...
ggplot (data = redwine, aes( density)) +
  geom_histogram(aes(x = density, fill = quality.group), binwidth = .0001) 
```
...
ggplot (data = redwine, aes(alcohol)) +
  geom_histogram(aes(x = alcohol, fill = quality), binwidth = .1) +
...
The above boxplot seems to suggest that as the alcohol content increases in the 
wine, it's quality increases.
However this cannot be said with surety since, we can see that some of the wines
of quality 5 have such high alcohol content.
...
ggplot(data = redwine, aes(fixed.acidity, density)) + 
  ...
  scale_color_discrete(labels = c('worst', 'normal', 'better')) +
  labs(color = 'quality_group') +
  geom_smooth() 
```

It looks like that the normal group tend to follow the smooth line roughly.
We can see major variations in the better group, but all in all they somehow 
follow the smooth line. Gives us indications that density and fixed acidity are
in fact correlated.
...
We can see that the worst group seems to vary a lot over the course. Albeit, If
we look at the normal and better groups we can see that the median for these 
groups tend to follow the geom smooth line. 
...
ggplot(data = redwine, aes(fixed.acidity, pH)) +
  geom_line(aes(color = redwine$quality.group), stat = 'summary',
    fun.y = 'median') +
  geom_line(stat = 'summary', fun.y = mean, linetype = 2, alpha = .75) +
  scale_color_discrete(labels = c('worst', 'normal', 'better')) +
...
We can see that there are a lot of variations in all groups of wine.
...
Sp it turns out that 65 percent of variance in density is explained by alcohol 
...
ggplot(data = redwine, aes(alcohol / fixed.acidity, density)) +
  ...
  ggtitle(label = "Density vs Alcohol and Fixed Acidity by Qulaity group")
```
...
content of the wine and it's fixed acidity. We can as the concentration of the alcohol over fixed acidity increases, the density tend to decrease. We can very high concentration of the points 

### Plot Two

```{r Plot_two, warning=FALSE, echo=FALSE}
alcohol.by_quality.groups <- redwine %>%
  group_by(quality.group) %>%
  summarize(Mean = mean(alcohol), Median = median(alcohol))
```


```{r Plot_Two, echo=FALSE}
ggplot (data = redwine, aes(alcohol)) +
  geom_histogram(aes(x = alcohol, fill = quality.group), binwidth = .1) + 
  geom_vline(data = alcohol.by_quality.groups, linetype = 2, mapping = 
               aes(color = quality.group, xintercept = Median)) +
  scale_fill_discrete(labels = c('worst', 'normal', 'better')) +
  scale_color_discrete(labels = c('worst', 'normal', 'better')) +
  labs(color = "Quality Groups", fill = "Quality Group") +
  ylab("No of Wines") +
  xlab("Concentration of Alcohol in the Wine") +
  ggtitle("Histogram of alcohol by Quality groups")

```

### Description Two
The above plot contains a lot of information. It is a histogram of alcohol by the quality groups. It has six vertical lines defining the mean and median of alcohol in all three groups of wine. It clearly suggests that wines of better quality tend to have higher concentration of alcohol. 
...
# Reflection
Red Wine data set contained 11 physicochemical variables. I have explored all the 
variable against the output i.e the quality of the wine. While some variables 
like, alcohol, sulphates tend to have a effect on the quality of the wine, some 
didn't. 
...
The relationship between the density of the wine and the Alcohol concentration plus the fixed acidity comes to me as a surprise. It wasn't expected. This is what you get when you explore the data. For a chemist this may seems like a predefined
relation, but for a guy with very limited knowledge with the chemistry of fluids this relation is a revelation. 
...
could explore the data even further by taking complex combinations of alcohol and 
sulphates at a time. These two variable have shown to have stronger correlation 
with quality of the wine that any other variables. 
